...
function FindMainFormComponent<T: TComponent>(const AName: string): T;
begin
  Result := T(frmWL2Main.FindComponent(AName));
  if Result = nil then
    raise Exception.CreateFmt('Component not found: %s', [AName]);
end;

procedure Translate(const aLangId: integer);
var
  i : integer;
  lGroupCaption: string;
begin
  WL2.Resources.VideoLink := WL2.Resources.VideoInstructionLink[aLangId];
  WL2.Resources.NoBioText := WL2.Resources.NoBiographyText[aLangId];
  with WL2.SavegameEditor.frmWL2Main do
    begin
      ...
      FindMainFormComponent<TGroupBox>(WL2.Resources.GroupNames[0]).Caption := WL2.Resources.GeneralInfoAdditionalLabels[0][aLangId];

      for i := 1 to 6 do  // Buttons
        FindMainFormComponent<TButton>(WL2.Resources.ButtonsNamesList[i]).Caption := WL2.Resources.ButtonsCaptions[i][aLangId];

      for i := 1 to 40 do  // Edit'es
        FindMainFormComponent<TEdit>(WL2.Resources.EditsNamesList[i]).Text := WL2.Resources.UnitDataCaptions[i][aLangId];

      for i := 1 to 12 do  // GroupBox'es
      begin
        if i in [1, 4, 7, 10] then
          lGroupCaption := WL2.Resources.GeneralInfoAdditionalLabels[1][aLangId]
        else if i in [2, 5, 8, 11] then
          lGroupCaption := WL2.Resources.GeneralInfoAdditionalLabels[2][aLangId]
        else
          lGroupCaption := WL2.Resources.GeneralInfoAdditionalLabels[3][aLangId];

        FindMainFormComponent<TGroupBox>(WL2.Resources.GroupNames[i]).Caption := lGroupCaption;
      end;

      for i := 1 to 6 do  // Labels
        FindMainFormComponent<TLabel>(WL2.Resources.LabelsNamesList[i]).Caption := WL2.Resources.GeneralInfoLabels[i][aLangId];

      for i := 1 to 8 do  // MenuItems
        FindMainFormComponent<TMenuItem>(WL2.Resources.MenuItemsNames[i]).Caption := WL2.Resources.MenuItemsCaptions[i][aLangId];

      for i := 1 to 4 do  // TabSheets
        FindMainFormComponent<TTabSheet>(WL2.Resources.ProgressBarsGroups[i]).Caption := WL2.Resources.SkillsGroupsCaptions[i][aLangId];
    end;
end;
